...
@onready var title_text   = $DialogueUI/DialogueBox/Title
...
func _ready():
	...
	scene_tween.tween_callback(func(): anim_player.play("final_cutscene"))
...
func show_title(speaker: String, text: String):
	speaker_label.text = speaker
	dialogue_label.text = ""
	if fade_tween:  fade_tween.kill()
	if typing_tween: typing_tween.kill()
